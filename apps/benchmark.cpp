...
int main(){
    std::ofstream csv("day15_fp32_b1.csv");
    csv<<"run_ID,Preprocess,Host Infer,Core E2E,NMS,PROCESS,REVERSE,H2D,TRT,D2H\n";
...
//前处理
    ...
    //runengine
        ...
        if(!trtengine.infer(input.data(),output.data(),&timing1))
        {
            std::cout<<"推理出错"<<std::endl;
            return -1;
        }
